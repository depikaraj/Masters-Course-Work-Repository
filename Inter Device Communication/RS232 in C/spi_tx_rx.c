...
void main()
{
	...
	while(1)
	{
		...
		if(kbhit())
		{
			...
			//if(ch == '0')
			rx_ch=0;
			printf("Sending %c\t",ch); //Display character
		 ...
		 else
		 {
			for(i=0;i<8;i++)
			{
			outportb(PORT+4,0x01);		       //	if(inportb(PORT+6)& 0x20==0x2);//clock high
			//delay(1);
//			MCR=MCR & 0xFE;
			//for(i=0;i<8;i++)
					if(inportb(PORT+6) & 0x30 == 0x30)
					{
				       printf("RX : 1\n");
					rx_ch+=pow(2,i);
					}
       ...
       //	 outportb(PORT+4,0X00);
			delay(1);
			outportb(PORT+4,0x00);//clock low
			delay(1);
			}
			printf("rx %c\n",rx_ch);
			rx_ch=0;
