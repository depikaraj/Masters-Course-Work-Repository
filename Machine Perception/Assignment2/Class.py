...
#InPut images to check if it was horse or a bike
...
# Load the classifier, class names, scaler, number of clusters and vocabulary which was trained earlier
...
k = 500  # Loading the number of cluster for k mens clustering
...
# Put all the descriptors vertically in a numpy array
descriptors = des_list[0][1]
for image_path, descriptor in des_list[0:]:
    descriptors = np.vstack((descriptors, descriptor))   # Store the descriptors in to a numpy array
...
nbr_occurences = np.sum( (test_features > 0) * 1, axis = 0)  # Getting frequency of each word
...
# Finding the 17 nearest neighbours of the test image descriptor k = 17

#Binary Classification whether Bike or Horse
...
if args["visualize"]:
    ...
    cv2.putText(image, prediction, pt ,cv2.FONT_HERSHEY_SCRIPT_SIMPLEX,2, [0, 0, 255],2)  # Placing text on the image, classification can be seen on th eimage itself
